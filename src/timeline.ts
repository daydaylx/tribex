/**
 * timeline.ts - Timeline-Komponente für TribeX
 * Visuelle Darstellung der Chain-Sequenz
 */

import { ProjectState, Pattern } from './tribeX-types';

class Timeline {
  private container: HTMLElement;
  private app: any;
  private element: HTMLElement | null = null;
  private patternElements: HTMLElement[] = [];
  ...
  private createTimelineElement(): void {
  ...
  render(): void {
    ...
    if (!chainContainer || !this.element) return;

    // Clear existing elements
    chainContainer.innerHTML = '';

    const project: ProjectState | null = this.app.state?.project;
    if (!project || !project.chain) return;

    // Create visual representation of each pattern in the chain
    project.chain.forEach((entry: any, index: number) => {
      // In a real implementation, entry would have a patternId property
      // For now, we'll assume entry is the patternId
      const patternId = typeof entry === 'object' ? entry.patternId : entry;
  ...
  private createPatternElement(pattern: Pattern, index: number): HTMLElement {
    ...
    // Add event listeners
    element.querySelector('.timeline-btn-move-left')?.addEventListener('click', (e: Event) => {
      e.stopPropagation();
      this.movePattern(index, -1);
    });
    
    element.querySelector('.timeline-btn-move-right')?.addEventListener('click', (e: Event) => {
      e.stopPropagation();
      this.movePattern(index, 1);
    });
    
    element.querySelector('.timeline-btn-remove')?.addEventListener('click', (e: Event) => {
      e.stopPropagation();
  ...
  private renderPatternSteps(pattern: Pattern): string {
  ...
  private addCurrentPatternToChain(): void {
  ...
  private movePattern(fromIndex: number, direction: number): void {
    if (!this.app.state?.project || !this.app.state.project.chain) return;
  ...
  updateForCurrentStep(stepIndex: number): void {
    ...
    document.querySelectorAll('.timeline-pattern .timeline-step.current').forEach((step: Element) => {
      step.classList.remove('current');
    });

    // Highlight the current step in the current pattern
    if (this.app.state?.currentPatternId) {
      const currentPatternElement = document.querySelector(`.timeline-pattern[data-pattern-id="${this.app.state.currentPatternId}"]`) as HTMLElement;
      if (currentPatternElement) {
